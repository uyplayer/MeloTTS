...
from melo.text.ug_utils.tokenizer.base_tokenizer import BaseTokenizer
import torch
import warnings
from abc import ABC
...
class XLMRobertaTokenizer(BaseTokenizer, ABC):
    ...
    def __init__(self):
        print(f"XLMRobertaTokenizer is initialized")
        self.device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        self.tokenizer = AutoTokenizer.from_pretrained('xlm-roberta-base', use_fast=True)
        print(f"Device type is {self.device.type}")
        print(f"XLM-RoBERTa model is loaded")
